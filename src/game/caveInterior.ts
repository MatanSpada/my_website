...
export function buildCaveInterior(scene:Scene,stone:StandardMaterial,wood:StandardMaterial){
  ...
  for(const [x,z,i] of positions){const label=SPECIALIZATIONS[i];const door=box(scene,`specialization-${i}`,x,2.2,z,2.7,4.4,.35,wood);const plaque=MeshBuilder.CreatePlane(`plaque-${i}`,{width:2.45,height:.55},scene);plaque.position.set(x,4.9,z+.2);plaque.rotation.y=z<-26?Math.PI:0;const tex=new DynamicTexture(`label-${i}`,{width:1024,height:256},scene,false);tex.drawText(label,28,155,"bold 54px Arial","#e5d2a2","#1b211f",true);const mat=new StandardMaterial(`plaque-mat-${i}`,scene);mat.diffuseTexture=tex;mat.emissiveColor=new Color3(.15,.12,.07);plaque.material=mat;doors.push({id:`specialization-${i}`,label,position:door.position.clone(),state:"CLOSED",collisionEnabled:true});}
