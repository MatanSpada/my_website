...
export type WorldInfo = { name: "cave-entrance" | "chamber"; spawn: Vector3; yaw: number; door?: MainDoor };
...
export function buildCaveEntrance(scene: Scene): WorldInfo {
  ...
  const ground = material(scene, "wet-slate-ground", new Color3(.075, .085, .08));
  const stone = material(scene, "facade-stone", new Color3(.105, .12, .125));
  ...
  staticBox(scene, "cave-depth", 0, 3.1, -8.3, 5.2, 6.2, 2.4, darkStone);
  ...
  const cool = new HemisphericLight("cold-night", new Vector3(.1, 1, .2), scene); cool.diffuse = new Color3(.36, .46, .58); cool.groundColor = new Color3(.03, .04, .055); cool.intensity = .82;
  const guide = new PointLight("entrance-glow", new Vector3(0, 3.7, -2.8), scene); guide.diffuse = new Color3(.82, .54, .3); guide.intensity = 78; guide.range = 17;
  return { name: "cave-entrance", spawn: new Vector3(0, .9, 9), yaw: Math.PI, door };
}
