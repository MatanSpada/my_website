...
export class PhysicalPlayer {
  ...
  update(dt: number, input: InputState, yaw: number, profile: MovementProfile) { const direction = normalizedInput(input); const forward = new Vector3(Math.sin(yaw), 0, Math.cos(yaw)); const right = new Vector3(Math.cos(yaw), 0, -Math.sin(yaw)); const wanted = forward.scale(direction.z).add(right.scale(direction.x)).scale(profile.maxSpeed); const rate = wanted.lengthSquared() ? profile.acceleration : profile.deceleration; this.velocity.x = approach(this.velocity.x, wanted.x, rate, dt); this.velocity.z = approach(this.velocity.z, wanted.z, rate, dt); const previous = this.position.clone(); const physicalVelocity = this.body.body.getLinearVelocity(); this.body.body.setLinearVelocity(new Vector3(this.velocity.x, physicalVelocity.y, this.velocity.z)); this.position.copyFrom(this.body.transformNode.position); this.velocity.copyFrom(this.body.body.getLinearVelocity()); this.grounded = this.position.y <= 1.02 && this.velocity.y <= .15; return { speed: Math.hypot(this.velocity.x, this.velocity.z), actualMovement: Math.hypot(previous.x - this.position.x, previous.z - this.position.z) > .0001 };
  }
