...
export class PhysicalPlayer {
  ...
  constructor(scene: Scene) { const capsule = MeshBuilder.CreateCapsule("player-body", { height: 1.8, radius: .32 }, scene); capsule.isVisible = false; capsule.position.copyFrom(this.position); this.body = new PhysicsAggregate(capsule, PhysicsShapeType.CAPSULE, { mass: 80, friction: .8, radius: .32, pointA: new Vector3(0, -.58, 0), pointB: new Vector3(0, .58, 0) }, scene); }
