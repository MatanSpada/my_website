...
export const LINUX_EBPF_EXHIBITS: readonly PortfolioExhibit[] = [
  { id:"linux-ebpf-identity", title:"LINUX KERNEL & eBPF", description:"Systems-focused technical chamber.", tags:["Linux","eBPF"], position:new Vector3(5,2.05,-23.65), rotationY:Math.PI },
  { id:"ebpf-execution-flow", title:"eBPF EXECUTION FLOW", description:"Kernel hooks · programs · events", tags:["Kernel","Events"], position:new Vector3(4.05,2.05,-22.3), rotationY:-Math.PI/2 },
  { id:"kernel-userspace-pipeline", title:"KERNEL → USER SPACE", description:"A concise event-pipeline study.", tags:["Pipeline","User space"], position:new Vector3(6.95,2.05,-22.3), rotationY:Math.PI/2 }
];
...
const plaque=(scene:Scene,exhibit:PortfolioExhibit,bronze:StandardMaterial)=>{const face=MeshBuilder.CreatePlane(`${exhibit.id}-plaque`,{width:2.35,height:.72,sideOrientation:Mesh.DOUBLESIDE},scene);face.position.copyFrom(exhibit.position);face.rotation.y=exhibit.rotationY;const text=new DynamicTexture(`${exhibit.id}-text`,{width:1024,height:320},scene,false);text.drawText("",0,0,"1px Arial","transparent","#1c1710",true,false);text.drawText(exhibit.tags.join("  ·  "),512,275,"22px Arial","#b89054",null,true,false);text.drawText(exhibit.description,512,205,"26px Arial","#d5c9ab",null,true,false);text.drawText(exhibit.title,512,105,"600 40px Georgia","#ead9a8",null,true,true);const mat=new StandardMaterial(`${exhibit.id}-material`,scene);mat.diffuseTexture=text;mat.emissiveTexture=text;mat.disableLighting=true;face.material=mat;face.freezeWorldMatrix();};
