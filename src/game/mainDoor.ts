...
export function buildMainDoor(scene: Scene, position: Vector3): MainDoor {
  ...
  const createColliders=()=>{leaf=new PhysicsAggregate(slab,PhysicsShapeType.BOX,{mass:0,friction:.9},scene); sealMesh=MeshBuilder.CreateBox("main-door-closed-seal",{width:5.05,height:5.2,depth:.9},scene);sealMesh.position.set(position.x,2.6,position.z+.1);sealMesh.isVisible=false;seal=new PhysicsAggregate(sealMesh,PhysicsShapeType.BOX,{mass:0,friction:.95},scene);}; createColliders();
