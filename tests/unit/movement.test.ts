...
import { DEFAULT_PROFILE, profiles, selectedProfile } from "../../src/game/profile";
...
describe("movement foundations", () => {
  ...
  it("uses locked as the default and falls back there for invalid profiles", () => { expect(selectedProfile(null).name).toBe(DEFAULT_PROFILE); expect(selectedProfile("broken").name).toBe("locked"); });
  ...
  it("selects each requested profile and gives each measurable movement and camera behavior", () => {
    ...
    expect(selectedProfile("locked")).toBe(profiles.locked);
  ...
  it("locks an intermediate controlled response between grounded and responsive", () => {
    const dt=1/60;
    const afterTenFrames=(profile=profiles.grounded)=>{ let speed=0; for(let frame=0;frame<10;frame++) speed=approach(speed,profile.maxSpeed,profile.acceleration,dt); return speed; };
    const stoppedAfterSixFrames=(profile=profiles.grounded)=>{ let speed=profile.maxSpeed; for(let frame=0;frame<6;frame++) speed=approach(speed,0,profile.deceleration,dt); return speed; };
    expect(afterTenFrames(profiles.locked)).toBeGreaterThan(afterTenFrames(profiles.grounded));
    expect(afterTenFrames(profiles.locked)).toBeLessThan(afterTenFrames(profiles.responsive));
    expect(stoppedAfterSixFrames(profiles.locked)).toBeLessThan(stoppedAfterSixFrames(profiles.grounded));
    expect(stoppedAfterSixFrames(profiles.locked)).toBeGreaterThan(stoppedAfterSixFrames(profiles.responsive));
    expect(profiles.locked.lookResponse).toBeGreaterThan(profiles.grounded.lookResponse);
    expect(profiles.locked.lookResponse).toBeLessThan(profiles.responsive.lookResponse);
  });
